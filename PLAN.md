...
- [x] Step 8.5 — 硬件扩展工具 (PWM + ADC + Servo + OneWire，新增 18 个工具)
- [x] Step 8.6 — 补齐 zclaw 差距 (delay + memory_list + get_version + 增强诊断)
- [ ] Step 9 — 生产就绪 ← 当前
- [ ] Step 10 — S3 全功能
...
### 11.2 PWM 工具 ✅ (已完成)
**文件:** `hal/hal_pwm.h` + `.c`, `tool/tool_pwm.c`

**工具:**
- `pwm_setup` — 配置 PWM 通道 (频率 + 引脚)
...
### 11.3 ADC 工具 ✅ (已完成)
**文件:** `hal/hal_adc.h` + `.c`, `tool/tool_adc.c`

**工具:**
- `adc_read` — 读取 ADC 原始值 + 电压
...
### 11.3.5 Servo 工具 ✅ (已完成)
**文件:** `hal/hal_servo.h` + `.c`, `tool/tool_servo.c`

**工具:**
- `servo_attach` — 绑定舵机到引脚
- `servo_write` — 设置角度 (-90° ~ +90°)
- `servo_write_us` — 设置脉宽 (500-2500us)
- `servo_read` — 读取当前角度
- `servo_detach` — 释放舵机

**应用场景:** 智能窗帘、机械臂、云台摄像头

### 11.3.6 OneWire 温度工具 ✅ (已完成)
**文件:** `hal/hal_onewire.h` + `.c`, `tool/tool_onewire.c`

**工具:**
- `temp_setup` — 配置 OneWire 总线
- `temp_scan` — 扫描设备地址
- `temp_read` — 读取温度 (支持指定设备)
- `temp_release` — 释放总线

**应用场景:** DS18B20 防水温度传感器、环境监测
...
| P0 | PWM | ★★☆ | 调光/调速 (LED/电机) | ✅ 已完成 |
| P0 | ADC | ★★☆ | 模拟传感器 (光/温度/电量) | ✅ 已完成 |
| P0 | Servo | ★★★ | 舵机控制 (机械臂/窗帘) | ✅ 已完成 |
| P0 | OneWire | ★★★ | DS18B20温度传感器 | ✅ 已完成 |
...
| 工具 | 功能 | 参数 |
|------|------|------|
| `pwm_setup` | 配置 PWM 通道 | pin, channel(0-5), freq(100-40000Hz) |
| `pwm_set_duty` | 设置占空比 | channel, duty(0-100%) |
| `pwm_set_freq` | 设置频率 | channel, freq(Hz) |
| `pwm_stop` | 停止输出 | channel |
| `pwm_status` | 查询状态 | channel |
...
| 工具 | 功能 | 参数 |
|------|------|------|
| `adc_read` | 读取原始值+电压 | pin |
| `adc_read_voltage` | 读取电压(mV) | pin |
| `adc_read_avg` | 多次采样平均 | pin, samples(1-64) |
...
| 工具 | 功能 | 参数 |
|------|------|------|
| `servo_attach` | 绑定舵机到引脚 | pin, id(0-3) |
| `servo_write` | 设置角度 | id, angle(-90~+90°) |
| `servo_write_us` | 设置脉宽(微秒) | id, pulse_us(500-2500) |
| `servo_read` | 读取当前角度 | id |
| `servo_detach` | 释放舵机 | id |
...
| 工具 | 功能 | 参数 |
|------|------|------|
| `temp_setup` | 配置 OneWire 总线 | pin, bus(0-1) |
| `temp_scan` | 扫描设备地址 | bus |
| `temp_read` | 读取温度 | bus, rom(可选) |
| `temp_release` | 释放总线 | bus |

**应用:** DS18B20 防水温度传感器、环境监测
...
> 🔧 **硬件工具总数**: ESPClaw 提供 17 个硬件控制工具 (GPIO 3 + PWM 5 + ADC 3 + Servo 5 + OneWire 4 = 20 个，但 GPIO 是之前的)
