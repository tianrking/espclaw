...
static bool tool_pwm_stop(const char *input_json, char *result_buf, size_t result_sz)
{
    ...
    if (err != ESP_OK) {
        snprintf(result_buf, result_sz, "Error: failed to stop (%s)", esp_err_to_name(err));
        return false;
    }
...
static bool tool_pwm_status(const char *input_json, char *result_buf, size_t result_sz)
{
    ...
    uint32_t freq_hz, duty_pct;
    esp_err_t err = hal_pwm_get_status(channel, &freq_hz, &duty_pct);
    ...
    snprintf(result_buf, result_sz, "PWM ch%d: %lu Hz, %lu%% duty", channel, freq_hz, duty_pct);
    return true;
}
